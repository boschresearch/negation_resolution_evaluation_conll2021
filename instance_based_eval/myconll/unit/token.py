...
from myconll.conllable import Conllable
from myconll.exception import ParseError, FormatError
...
class Token(Conllable):
    ...
    # Modified code here: added slots
    # TODO: are we keeping the filename?
    # cue, scope, event will be tuples of ids of the negation instances to which the token belongs,
    # starting at 0
    __slots__ = [
        'filename', 'sent_num', 'id', '_form', 'lemma', 'xpos', 'parse', 'cue', 'scope', 'event'
    ]
    ...
    def __init__(self, source: str, empty: bool = False) -> None:
        ...
        # Modified: skipping this check because in the negation data, the number of cols varies
        #if len(fields) != 10:
        ...
        self.doc_id : str = fields[0]
        self.sent_id: str = fields[1]
        self.id: str = fields[2]
        ...
        # actual tokens.
        # Modified: changed offsets
        if empty or (fields[3] != Token.EMPTY or fields[4] != Token.EMPTY):
            self._form: Optional[str] = _unit_empty_map(fields[3], Token.EMPTY)
            self.lemma: Optional[str] = _unit_empty_map(fields[4], Token.EMPTY)
        else:
            self._form = fields[3]
            self.lemma = fields[4]

        self.xpos: Optional[str] = _unit_empty_map(fields[5], Token.EMPTY)
        self.parse: Optional[str] = _unit_empty_map(fields[6], Token.EMPTY)
        #self.upos: Optional[str] = _unit_empty_map(fields[5], Token.EMPTY) # added because writer threw error
        #self.feats: Optional[str] = _unit_empty_map(fields[5], Token.EMPTY)
     
        # Modified: reading in negation instances in pairs of 3 columns
        # the cue, scope, and event fields contain tuples of pairs: (negation_inst_id_within_sent, cue/scope/event)
        self.cue = None
        self.scope = None
        self.event = None
        if len(fields) == 8 and fields[7] == Token.PLACEHOLDER:            
            # nothing to do, no cue / scope to read
            pass
        elif len(fields) > 7:
            self.cue = []
            self.scope = []
            self.event = []
            neg_inst = 0
            for i in range(7, len(fields), 3):
                if fields[i] is not Token.EMPTY:
                    self.cue.append((neg_inst, fields[i]))
                if fields[i+1] is not Token.EMPTY:
                    self.scope.append((neg_inst, fields[i+1]))
                if fields[i+2] is not Token.EMPTY:
                    self.event.append((neg_inst, fields[i+2]))
                neg_inst += 1
            # make immutable for consistency with pyconll
            self.cue = tuple(self.cue)
            self.scope = tuple(self.scope)
            self.event = tuple(self.event)
    ...
    def conll(self) -> str:
        ...
        parse = _unit_conll_map(self.parse, Token.EMPTY)

        MAX_NEG_INST_PER_SENT = 4 # TODO set this globally from file instead
        neg_inst_items = []
        for i in range(MAX_NEG_INST_PER_SENT):
            cue = Token.EMPTY
            scope = Token.EMPTY
            event = Token.EMPTY
            if self.cue:
                for c in self.cue:
                    if c[0] == i:
                        # a negation instance exists for this index
                        cue = _unit_conll_map(c[1], Token.EMPTY)
            if self.scope:
                for s in self.scope:
                    if s[0] == i:
                        # a scope annotation exists for this index
                        scope = _unit_conll_map(s[1], Token.EMPTY)
            # TODO? events still broken?
            if self.event:
                for e in self.event:
                    if e[0] == i:
                        # a scope annotation exists for this index
                        scope = _unit_conll_map(e[1], Token.EMPTY)
            neg_inst_items += [cue, scope, event]
        ...
        # original pyconll version
        #items = [
        #    token_id, form, lemma, upos, xpos, feats, head, deprel, deps, misc
        #]
        # myconll for negation format version
        items = [
            doc_id, sent_id, token_id, form, lemma, xpos, parse
        ] + neg_inst_items
